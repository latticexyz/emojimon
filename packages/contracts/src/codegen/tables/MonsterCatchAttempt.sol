...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_OFFCHAIN_TABLE, bytes14(""), bytes16("MonsterCatchAtte")))
);
ResourceId constant MonsterCatchAttemptTableId = _tableId;
...
library MonsterCatchAttempt {
  ...
  function decodeStatic(bytes memory _blob) internal pure returns (MonsterCatchResult result) {
    result = MonsterCatchResult(uint8(Bytes.slice1(_blob, 0)));
  }
   ...
   */
  function encode(MonsterCatchResult result) internal pure returns (bytes memory, PackedCounter, bytes memory) {
    bytes memory _staticData = encodeStatic(result);
